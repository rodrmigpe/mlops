...
def test_model_type(data):
    model, rmse = train_model(data)
    assert isinstance(model, RandomForestRegressor) or isinstance(model, GradientBoostingRegressor) or isinstance(model, LinearRegression) or isinstance(model, XGBRegressor) or isinstance(model, DecisionTreeRegressor) or isinstance(model, MLPRegressor) or isinstance(model, AdaBoostRegressor) or isinstance(model, CatBoostRegressor) or isinstance(model, PolynomialFeatures) or isinstance(model, StackingRegressor), "train_model should return either RandomForestRegressor or GradientBoostingRegressor or LinearRegression or KNeighborsRegressor or XGBRegressor or DecisionTreeRegressor or MLPRegressor or AdaBoostRegressor or CatBoostRegressor or PolynomialFeatures or StackingRegressor"
...
def train_model(data):
    ...
    #svr_model, mean_rmse_scores_svr = svr_regressor(x_train, y_train, kf) #Support Vector Machine Regressor
    ...
    #mean_rmse_svr = round(pd.Series(mean_rmse_scores_svr).mean(), 4)
...
def svr_regressor(x_train, y_train, kf):
    # Create lists to store mean scores
    mean_rmse_scores = []
    ...
    for train_index, test_index in kf.split(x_train, y_train):
        ...
        # Make predictions on this fold's testing data
        svr_pred_fold = svr_model.predict(X_test_fold)
        
        # Calculate the metrics for this fold's predictions
        svr_rmse_fold = mean_squared_error(y_test_fold, svr_pred_fold, squared=False)
        
        # Append the mean score for this fold to the appropriate list
        mean_rmse_scores.append(svr_rmse_fold)
        
    return svr_model, mean_rmse_scores
